...
const fromSource = sourceIndex => new Game(LEVELS.findIndex(level => level.sourceIndex === sourceIndex))
...
assert(LEVELS.every((level, index) => level.id === `${Math.floor(index / 5) + 1}-${String(index % 5 + 1).padStart(2, '0')}`))
...
const chapterCells = chapter => LEVELS.filter(level => level.chapter === chapter).flatMap(level => level.tiles).join('')
...
assert(!gameSource.includes('depth25d') && gameSource.includes("ctx.translate(tile * .07, tile * .11)"))
...
assert.equal([...gameSource.matchAll(/track\('game_start'/g)].length, 1)
assert(gameSource.includes("source: scene === 'title' ? 'new' : 'returning'") && gameSource.includes("query: 'source=menu'") && gameSource.includes("query?.source !== 'menu'"))
...
assert(!gameSource.includes('getSystemInfoSync') && gameSource.includes('wx.getWindowInfo()') && gameSource.includes('wx.getDeviceInfo()'))
assert(gameSource.includes('function resetTeaching()') && gameSource.includes("button('重看机关说明'"))
assert(['wx.onError', 'wx.onUnhandledRejection', 'wx.onAudioInterruptionBegin', 'wx.onAudioInterruptionEnd'].every(api => gameSource.includes(api)))
...
LEVELS.forEach(level => {
  ...
  if (level.difficulty !== '入门' && level.id !== '4-01') assert(level.width >= 18 && level.width <= 24 && level.height >= 7 && level.height <= 10, `${level.id} should use the 18-22 x 7-9 standard or the 24 x 10 special size`)
  const ids = new Set()
...
assert(LEVELS.every(level => level.status === 'playtest'), 'all levels must pass playtest before release')
assert.equal(Object.keys(SOLUTIONS).length, LEVELS.length, 'every level must have exactly one official solution')
