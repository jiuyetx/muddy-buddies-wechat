...
assert.deepEqual(positions(basic.worm), [[2, 2], [2, 3], [3, 3]])

const rock = new Game(5)
rock.worm = [[5, 2], [4, 2]]
assert.equal(rock.move('right'), true)
assert(rock.rocks.has('7,2'))

const cut = new Game(3)
...
assert.deepEqual(body.worm[1].slice(), [2, 2])
assert.equal(body.worm[1].state.charged, true)
assert.equal(body.worm[0].next, middleId)
assert.equal(body.segmentAt([2, 2]).id, middleId)
